...
- **The frontend is not containerized, and its API URL is hardcoded to `localhost` for local development** (`src/environments/environment.ts`) — Docker was only required for the backend per the assessment spec, and there is no deployment target for the frontend today, so a production environment file was never needed. See "Potential Improvements" below.
...
- **Ship logs to a centralized log server** such as [Seq](https://datalust.co/seq) (Serilog's own tooling has a first-class sink for it) instead of only console + local rolling files. Today's file sink is fine for a single instance running on one machine, but doesn't scale to querying/filtering structured fields across multiple instances or correlating a request across services — which is exactly what Seq (or an equivalent like the ELK/EFK stack, or a cloud APM) is for.
- **A CD pipeline deploying to a real environment** (e.g., Azure). The backend would build/push its existing Docker image to a registry (Azure Container Registry) and deploy it to Azure App Service for Containers, or publish directly with no container at all via `dotnet publish` + `azure/webapps-deploy`; the frontend, being static output, would suit Azure Static Web Apps better than a container. The frontend piece would also need an `environment.prod.ts` (with `fileReplacements` wired up in `angular.json`) so the production build points at the deployed API instead of `localhost` — Angular bakes that URL in at build time since a static SPA has no server-side runtime to read configuration from at request time, unlike the backend.
---
